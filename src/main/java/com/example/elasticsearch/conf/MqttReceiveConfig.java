...
public class MqttReceiveConfig {
    ...
    public MessageProducer inbound() {
        adapter = new MqttPahoMessageDrivenChannelAdapter(clientId + "_inbound", mqttClientFactory(), defaultTopic);
//        adapter = new MqttPahoMessageDrivenChannelAdapter(clientId + "_inbound", mqttClientFactory(), "");
//        String[] topicArr = defaultTopic.split(",");
...
//                String type = topic.substring(topic.lastIndexOf("/") + 1, topic.length());
                ...
                if ("monitorData".equalsIgnoreCase(topic)) {
                    System.out.println("订阅主题: " + topic);
//                    AsyncTask asyncTask = new AsyncTask();
//                    asyncTask.dataDeal(payLoad);
                }
            }
        };
    }
